    ...
    data.frame(coredata(originali)) %>%
      mutate(time=index(originali),alterati=round(as.vector(dati.xts[righeInvalidare,]),1) ) %>%
        select(time,alterati,everything()) %>%
          write_delim(.,path=paste0(debug,codice,"_",parametro,"_valoriAlterati.csv"),delim=";",col_names=TRUE)
    	...
    	if(nchar(extraOut)){
    	  as.data.frame(media.target.lim) %>%
    	    mutate(time=row.names(media.target.lim)) %>%
        	  write_delim(.,delim=";",col_names =TRUE,path=paste0(extraOut,codice,"_",parametro,"_matriceMedie.corroboration.csv"))
    	} #su nchar(extraOut)
...
tempRegression_test<-function(dati.xts,
  ...
  mapply(FUN=function(inizio.mese,fine.mese){
    ...
    if(nchar(debug)){
      df.per.debug %>% 
        write_delim(.,delim=";",col_names=TRUE,path=paste0(debug,prefisso,".regressionAlterati.csv"))
    }
